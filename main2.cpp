...
static string start;
static string endd;
map<string, int> nodetoedgeidx;
map<string, int> nodetonodeidx;
...
void *job(void *args){
	...
	while(1){
        ...
        int i = nodetoedgeidx.find(n->ID)->second;
		for(; i < edges.size() && edges[i].start == n->ID; i++){
            int j = nodetonodeidx.find(edges[i].end)->second;
			int flag = 0;
...
int main(int argc, char *argv[]){
	...
	while(fgets(str,256,fp)){
        ...
        if(e.start != currentstart){
            nodetoedgeidx.insert({e.start, index});
